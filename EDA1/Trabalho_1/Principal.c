              ...
              switch(caractere_atual) {
                 ...
                 case 43 ... 47: 
                      ...
                      if (pilha_vazia(p) || aux == 40) {
                         ...
                         while ( le_topo(p, &aux) != ERRO_PILHA_VAZIA){  //  while ( le_topo(p, &aux) != ERRO_PILHA_VAZIA && precedencia(&aux) >= precedencia (&caractere_atual)) REVER A FUN��O PRECED�NCIA PARA FUNCIONAR
                                desempilha(&p, &aux);
                                saida[j] = aux;
                         }
                         empilha(&p, caractere_atual);
                 ...
                 break;
                 default:
                     printf("Erro: caractere inv�lido: %c!", caractere_atual); 
              }
          }
          if(deu_erro == 0) {
            while(le_topo(p, &aux) != ERRO_PILHA_VAZIA && aux != 40) {
